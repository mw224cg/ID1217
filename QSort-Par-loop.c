...
-Pivot elementet väljs alltid i mitten av arrayen
-Tröskel uppnådd => två tasks skapas som sedan fångas av #pragma omp taskwait*/
...
/* Returnerar ett random index från ett intervall low-high
low (int): startindex för sub-arrayen
high (int): slutindex för sub-arrayen
Returns: index för valt pivot-element
*/
int chose_random_element(int low, int high){
...
void quick_sort(int arr[], int low, int high){
    ...
    if(low < high){
        ...
        if(task_threshold < high - low){ //tillräckligt stort problem => tasks
            ...
            #pragma omp taskwait        //vänta tills tasks klara
...
int main(int argc, char* argv[]){
    ...
    {
    case 1:
        printf("Usage: ./QSPLoop + 'number of cores' + 'array size'\n");
    ...
    task_threshold = TASK_THRESHOLD; //byt till arr_size / (num_threads *25); för dynamisk
    ...
    printf("Average time: %f - Number of cores: %d\n", avg_time, num_threads);
